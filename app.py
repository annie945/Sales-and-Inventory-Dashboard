...
if page == "📦 Inventory & Risk":
    ...
    try:
        ...
        df_po_filtered = df_po[df_po['DESTINATION'].astype(str).str.contains(region_key, case=False, na=False)]
    ...
    st.divider()
    c1, c2 = st.columns(2)
    with c1: st.subheader("🔴 Out of Stock"); st.dataframe(s_df[s_df["Stock"]==0], hide_index=True)
    with c2: st.subheader("🟡 Low Stock (<50)"); st.dataframe(s_df[(s_df["Stock"]>0)&(s_df["Stock"]<50)], hide_index=True)
...
# --- 2. SALES ---
    ...
    try:
        ...
        df = df[df[s_col].apply(is_valid_sku)]; df['quantity'] = pd.to_numeric(df[q_col], errors='coerce').fillna(0)
        ...
        c_sum = curr_w.groupby(s_col)['quantity'].sum().reset_index().rename(columns={'quantity':'Units'})
        
        st.info(f"📍 Period: April 27 - May 3")
        m1, m2 = st.columns(2)
        m1.metric("📸 Cameras", f"{int(c_sum[c_sum[s_col].apply(is_cam)]['Units'].sum())}")
        m2.metric("🎒 Accessories", f"{int(c_sum[~c_sum[s_col].apply(is_cam)]['Units'].sum())}")
        
        st.divider(); st.subheader("📉 Bottom 5 Sellers (Weekly)"); st.dataframe(c_sum.nsmallest(5, 'Units'), hide_index=True)
...
# --- 3. LOGISTICS & REGION ANALYSIS ---
    ...
    with t_sum:
        try:
            ...
            latest = df_sum.groupby(df_sum[0].dt.to_period('M'))[[cols["fulfill"], cols["shipping"], cols["storage"]]].sum().iloc[-1]
            c1, c2, c3 = st.columns(3)
            c1.metric("Warehouse Fulfillment", f"{cur}{latest[cols['fulfill']]:,.2f}")
            c2.metric("Shipping Total", f"{cur}{latest[cols['shipping']]:,.2f}")
            c3.metric("Storage", f"{cur}{latest[cols['storage']]:,.2f}")
        except: st.error("Summary error.")
    ...
    with t_ship:
        try:
            ...
            if reg_3pl in ["🇺🇸 US", "🇨🇦 CA"]:
                raw['Macro'] = raw[4].apply(extract_state).map(US_MACRO if reg_3pl=="🇺🇸 US" else CA_MACRO).fillna('Other')
                ...
                with c2: st.dataframe(dist, hide_index=True)
            else:
                raw['Country'] = raw[14].apply(lambda x: normalize_loc(x, reg_3pl))
                st.dataframe(raw.groupby('Country')[order_col].nunique().reset_index().rename(columns={order_col:'Orders'}).sort_values(by='Orders', ascending=False), hide_index=True)
        except: st.error("Regional data error.")
